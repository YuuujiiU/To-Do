...
class SignUpVM:NSObject{
    ...
    func proceedWithRegister(completion: @escaping CompletionHandlerWithData) {
        
        validateSignUpUser(completion: { status, message in
            if status{
    ...
    func validateForm() throws -> Bool {
        ...
        guard !(confirmPassword.value.isEmpty) else {
            throw ValidateError.invalidData("Confirm Password is empty")
        }
        
        guard (confirmPassword.value == password.value) else {
            throw ValidateError.invalidData("Passoword Doesnt match")
        }
        return true
    }
